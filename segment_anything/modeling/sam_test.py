...
# from .image_encoder import ImageEncoderViT, MaskDecoder, TwoWayTransformer
from .image_encoder_test import ImageEncoderViT_test
...
#@register('sam')
class SAM(nn.Module):
    def __init__(self, image_encoder=ImageEncoderViT_test,prompt_encoder=PromptEncoder,mask_decoder=MaskDecoder,pixel_mean = [123.675, 116.28, 103.53],
    ...
    def set_input(self, input, spec, gt_mask):
        ...
        B, C, H, W = self.input.size()
        self.input = self.input.reshape(B,C, H, W)

        self.spec = spec.to(self.device)
        B, C_a, H_a, W_a = self.spec.size()
        self.spec = self.spec.reshape(B, C_a, H_a,W_a)

        
        #### vggish audio encoder
        with torch.no_grad():
            spec_out = self.audio_encoder(self.spec)#.reshape(B*W_a, -1)    # Bx128
            self.spec = spec_out.detach().to(self.device)
            
        B, H_m, W_m = gt_mask.size()
        gt_mask = gt_mask.reshape(B, H_m, W_m)
